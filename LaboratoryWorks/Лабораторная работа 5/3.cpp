...
int main(int argc, const char * argv[]) {
    ...
    ac = pow((pow(x3 - x1, 2) + pow(y3 - y1, 2)), 0.5); //Считаем расстояние по формуле, используя функцию pow(x, y) из библиотеки math.h
    bc = pow((pow(x3 - x2, 2) + pow(y3 - y2, 2)), 0.5);
    
    cout << "Произведение расстояний между AB и BC: " << ac * bc << endl;
